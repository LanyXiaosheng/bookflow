...
impl SettingsRepo {
    ...
    pub async fn read(&self) -> Result<Option<Settings>> {
        let row = sqlx::query_as::<_, (String, String, String, String, i32)>(
            "SELECT provider, base_url, api_key, model, timeout_secs FROM app_settings WHERE id = TRUE",
        )
        .fetch_optional(&self.pool)
        .await
        .context("读 app_settings 失败")?;

        let Some((provider, base_url, api_key, model, timeout_secs)) = row else {
            return Ok(None);
        };
        ...
        Ok(Some(Settings { provider, base_url, api_key, model, timeout_secs }))
    }
    ...
    pub async fn upsert(&self, s: &Settings) -> Result<Settings> {
        let row = sqlx::query_as::<_, (String, String, String, String, i32)>(
            r#"
            UPDATE app_settings
            SET provider = $1, base_url = $2, api_key = $3, model = $4, timeout_secs = $5
            WHERE id = TRUE
            RETURNING provider, base_url, api_key, model, timeout_secs
            "#,
        )
        .bind(&s.provider)
        .bind(&s.base_url)
        ...
        let (provider, base_url, api_key, model, timeout_secs) = row;
        Ok(Settings { provider, base_url, api_key, model, timeout_secs })
    }
