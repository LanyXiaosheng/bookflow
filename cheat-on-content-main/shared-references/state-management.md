...
| `enabled_perf_adapters` | array of string | perf-data adapter 名列表（如 `["douyin-session"]` 或 `["fanqie-session"]`）。空 → cheat-retro 走 manual paste | cheat-init / 用户手动配置后 | cheat-retro |
