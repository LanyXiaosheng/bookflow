...
### Added — 番茄小说 adapter（fanqie-session）

**动机**：支持番茄小说（fanqienovel.com）创作者的数据回收与预测闭环。

- **新 adapter `adapters/perf-data/fanqie-session/`** — Playwright 方案，结构与 douyin-session 一致（crawler.py / renderer.py / review.py / paths.py / run.sh）
- **cheat-publish** 新增 URL 模式识别：`fanqienovel.com/*` / `fanqie.com/*` → platform `fanqie`
- **cheat-retro** Platform 路由表新增 `fanqie → fanqie-session/run.sh <work_id>`
- **cheat-init** Q2.1 新增选项 d) 番茄小说，`enabled_perf_adapters` 派生 `["fanqie-session"]`
- **核心指标差异**：番茄小说以阅读量/收藏/评论/点赞/打赏为主，report.md 含派生比率（收藏/阅读、评论/阅读等）
- Cookie 隔离：`.auth-fanqie/`（与抖音 `.auth/` 分开）
