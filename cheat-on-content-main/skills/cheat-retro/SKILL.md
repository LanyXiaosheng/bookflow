...
| `fanqie` | `adapters/perf-data/fanqie-session/` | `bash <skills-dir>/../adapters/perf-data/fanqie-session/run.sh <work_id> <video_folder>` |
...
**fanqie-session 的特殊处理**：
- 作品 URL（如 `https://fanqienovel.com/page/7234567890123456789`）→ 提取 work_id
- 调用前确认 cookie 文件存在（adapter 会找 `.auth-fanqie/`）；不存在则提示用户先跑 `python <adapter>/crawler.py login`
- adapter 输出在 `<video_folder>/report.md`
- 番茄小说的核心指标是阅读量/收藏/评论/点赞/打赏，不是播放量——复盘段用"阅读"替代"播放"
...
2. **数据来源必须标注**。`数据来源: manual paste` 或 `数据来源: adapter:douyin-session` 或 `数据来源: adapter:fanqie-session` 写进复盘段
