...
> f) **番茄短篇**（5000-15000 字，单篇完结，番茄小说平台）— rubric 需 bump 为小说维度（完读率 / 试读区卡点 / 爆点密度）
> g) **番茄长篇**（连载，按章节更新，番茄小说平台）— 同上
> h) **其他**（游戏 / 美食 / 妆教 / 新闻 / 剧情）— 工作流通用，但 rubric 维度需要调
...
- 选 a → `false`
- 选 b/c/d/e/f/g/h/i → `true`，cheat-status 持续提示"你的形态可能需要 bump 调权重"
- **不再有"严重不匹配"档**——所有形态都能跑工作流，只是有的 rubric 需要更激进的 bump

**Q1.5: 典型时长**（仅 Q1=a/d/h 时问）
...
**Q1.5b: 典型字数**（仅 Q1=f/g 时问）

> "你的番茄小说典型字数？
> a) 5000-8000 字（短篇偏短）
> b) 8000-12000 字（短篇标准）
> c) 12000-15000 字（短篇偏长）
> d) 连载单章 1500-3000 字"

记录到 `typical_word_count`（6500 / 10000 / 13500 / 2000）。
...
> d) 番茄小说 — 装 fanqie-session adapter（Playwright + 登录番茄小说创作者中心）
...
1. **`.cheat-state.json`**
   ...
   ```json
   {
     ...
     "typical_duration_seconds": <Q1.5 派生：30/90/240/450/900；Q1=f/g 时为 null>,
     "typical_word_count": <Q1.5b 派生：6500/10000/13500/2000；Q1≠f/g 时为 null>,
     ...
     "enabled_perf_adapters": <Q2.1=a→[\"douyin-session\"]；b→[\"youtube-data-api\"]；c→[\"bilibili-stat\"]；d→[\"fanqie-session\"]；其他→[]>,
     "last_bump_at": null,
...
| `typical_duration_seconds` | Phase 3 | Q1.5 派生（Q1=f/g 时为 null） |
| `typical_word_count` | Phase 3 | Q1.5b 派生（Q1≠f/g 时为 null） |
...
| `enabled_perf_adapters` | Phase 3 | Q2.1 派生（如 Q2.1=a 则 `["douyin-session"]`；d 则 `["fanqie-session"]`） |
