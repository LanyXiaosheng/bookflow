...
test('localhost 访问会规范化到 127.0.0.1，避免记住我丢登录态', async ({ page }) => {
  await page.goto('http://localhost:5174/auth')
  await page.waitForURL('http://127.0.0.1:5174/auth')
})
...
test('勾选记住我后会记住邮箱', async ({ page }) => {
  ...
  await expect(page.locator('input[type="email"]')).toHaveValue('writer@example.com')
